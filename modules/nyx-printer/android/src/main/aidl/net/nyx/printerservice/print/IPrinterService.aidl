// Interface officielle du service d'impression NYX, reprise telle quelle du
// dépôt du constructeur (SZNoryox/PrinterClient) - le même constructeur que
// celui du terminal visé (Noryox NB55, service `com.incar.printerservice`).
//
// ┌──────────────────────────────────────────────────────────────────────────┐
// │ ON NE RÉÉCRIT PAS CE FICHIER, ON LE RECOPIE.                            │
// │                                                                          │
// │ Les codes de transaction AIDL sont POSITIONNELS : l'ordre des méthodes   │
// │ EST le contrat. La version précédente était reconstituée par             │
// │ rétro-ingénierie et ne déclarait que six `reserved` suivis de            │
// │ `printText`, ce qui suffisait à atteindre la 7e position mais interdisait │
// │ tout le reste du service. Insérer ou retirer une seule ligne ici décale  │
// │ toutes les méthodes suivantes, et le service exécutera une AUTRE méthode │
// │ que celle qu'on croit appeler.                                           │
// │                                                                          │
// │ Positions utiles : printText = 7, printEscposData = 19, printEndAutoOut  │
// │ = 21, getPrinterStatus = 4, openCashBox = 24.                            │
// └──────────────────────────────────────────────────────────────────────────┘
